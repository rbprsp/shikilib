...
using AnilibStorage = decltype(orm::make_storage(
    ...
    orm::make_table("anime", orm::make_column("id", &Anime::id, orm::primary_key()),
                    orm::make_column("name", &Anime::name),
...
using ShikiStorage = decltype(orm::make_storage(
    std::declval<std::string>(),
    orm::make_table(
        "anime", orm::make_column("target_id", &ShikiList::target_id),
        ...
        orm::make_column("score", &ShikiList::score),
        orm::make_column("status", &ShikiList::status),
        orm::make_column("rewatches", &ShikiList::rewatches),
        orm::make_column("episodes", &ShikiList::episodes),
        orm::make_column("text", &ShikiList::text))));

using MappingStorage = decltype(orm::make_storage(
    std::declval<std::string>(),
